...
import { DeliveryView } from "./DeliveryView.tsx";
  ...
  return <Stack gap="sm">
    ...
    {selected && <section aria-label={`Project details: ${selected.name}`}>
      <Stack gap="sm">
        ...
        <DeliveryView key={selected.id} project={selected} />
        <Title order={3}>Project notes and context</Title>
        <Text size="sm">Four-question notes are saved context, not the current delivery milestone.</Text>
        <CopyText text={projectWorkBrief(selected)} label="Copy project notes (context only)" />
        ...
        <ProjectCard key={selected.id} project={selected} busy={busyId === selected.id} onOverride={onOverride} onCorrect={onCorrect} />
      </Stack>
    </section>}
