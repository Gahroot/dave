...
import { collectDeliveryContext, CONTEXT_CATEGORIES, reviewableDocument, reviewDeliveryDocument, type ContextPermissions } from "../model/delivery-context.ts";
...
export function registerDeliveryRoutes(app: FastifyInstance, db: Db, inference: PlanningInference) {
  const repo = deliveryRepo(db), planner = deliveryPlanner(db, inference), coordination = deliveryCoordinationRepo(db);
  ...
  function permissions(value: unknown): ContextPermissions {
    ...
    if (value.additionalFiles !== undefined) {
      ...
      for (const file of value.additionalFiles) {
        ...
        if (!reviewableDocument(file.path) || typeof file.reviewedSha256 !== "string" || !/^[a-f0-9]{64}$/.test(file.reviewedSha256)) throw new DeliveryError("invalid", "Invalid file review");
      }
    }
  ...
  async function collect(id: string, revision: number, selection: ContextPermissions) {
    ...
    try {
      const packet = await collectDeliveryContext({ project, goal: state.goal, provider: state.goal.provider, model: state.goal.model, permissions: selection, history: state.history, plans: state.plans, coordination: coordination.read(id).coordination });
  ...
  async function start(id: string, revision: number, purpose: GenerationPurpose) {
    ...
    if (!coordination.read(id).coordination.goalMatches) throw new DeliveryError("invalid", "Update the finish contract to match the current goal before planning");
    if (coordination.read(id).coordination.contract && !state.goal?.consent?.categories.includes("history")) throw new DeliveryError("invalid", "Select history to include the finish contract and review a fresh context packet");
  ...
  function failure(error: unknown, reply: FastifyReply) {
    if (error instanceof CoordinationInputError || error instanceof PlanOutputError) return reply.code(400).send({ error: error.message });
  ...
  app.get<{ Params: { id: string } }>(base, async (req, reply) => {
    try { return { ...coordination.read(req.params.id), operation: planner.latest(req.params.id) }; } catch (error) { return failure(error, reply); }
  });
    ...
    app.post<{ Params: { id: string; milestoneId?: string } }>(`${base}/${action}`, async (req, reply) => {
      ...
      try {
        ...
        if (action.endsWith("complete")) {
          if (coordination.read(id).coordination.contract) throw new DeliveryError("invalid", "Submit a structured result and explicitly review it; legacy completion cannot advance a coordinated plan");
          ...
          const recovery = result.state.nextPlans.some(n => n.status === "pending") ? "Saved queue exhausted. Define a finish line and review evidence; no model request was made." : null;
          return { ...result, state: repo.read(id), source: "user-reported", operation: planner.latest(id), recovery };
        }
        const state = expected(id, revision);
        if (action === "goal") {
        ...
        if (coordination.read(id).coordination.contract) throw new DeliveryError("invalid", "Use the delivery review or block action for coordinated plans");
  ...
  app.post<{ Params: { id: string } }>(`${base}/document/review`, async (req, reply) => {
    try {
      strictObject(req.body, ["expectedRevision", "path"]);
      expected(req.params.id, req.body.expectedRevision);
      const project = db.prepare("SELECT p.canonical_path,COALESCE(o.hidden,0) AS hidden FROM projects p LEFT JOIN project_overrides o ON o.project_id=p.id WHERE p.id=?").get(req.params.id) as { canonical_path: string; hidden: number };
      if (project.hidden) throw new DeliveryError("invalid", "Unhide the project before reviewing documents");
      let document;
      try { document = await reviewDeliveryDocument(project.canonical_path, req.body.path); }
      catch { throw new DeliveryError("invalid", "Document unavailable, excluded, outside the project, unsafe or over 8KB"); }
      expected(req.params.id, req.body.expectedRevision);
      return document;
    } catch (error) { return failure(error, reply); }
  });
