...
it("requires exact server preview approval, persists operations on reload and completes exactly once with changed-history recovery", async () => {
  ...
  expect(saved.recovery).toContain("context-changed"); expect(saved.state.history.filter((e: { kind: string }) => e.kind === "complete")).toHaveLength(3);
...
it("block/reopen preserve reports and exhausted provider failure does not replay completion", async () => {
  const f = await fixture();
  ...
  await new Promise(resolve => setImmediate(resolve));
  const saved = (await f.get(f.base)).json(); expect(saved.operation.status).toBe("error"); expect(JSON.stringify(saved)).not.toContain("PRIVATE");
  expect(saved.state.history.filter((e: { kind: string }) => e.kind === "complete")).toHaveLength(3);
  expect((await f.post(lastUrl, lastBody)).json().duplicate).toBe(true); expect(f.infer).toHaveBeenCalledTimes(2);
